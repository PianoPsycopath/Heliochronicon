...
def process_hyg_csv(input_csv: Path, output_dir: Path):
    ...
    print(f"Finished reading. Grouped into {len(constellations)} constellations. Skipped {skipped} rows.")
    ...
    for con, records in constellations.items():
        ...
        print(f" -> Completed {dataset_name}: {len(records)} records across {len(chunk_filenames)} chunks.")
