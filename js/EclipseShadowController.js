...
export class EclipseShadowController {
    constructor(ctx) { this.ctx = ctx; this.overlays = new Map(); }
    ...
    _findStarBody(bodyObj) {
        ...
        while (current && !visited.has(current.name)) {
            ...
            if (current.parent === current.name) return bodies.find(b => b.data.name === current.name) || null;
    ...
    _candidates(bodyObj) {
        ...
        return this.ctx.celestialBodies.filter(b =>
            b !== bodyObj && b.data.parent !== b.data.name &&
    ...
    onMeshVisibilityChange(bodyObj, isVisible) {
        ...
        if (!isVisible) { const e = this.overlays.get(bodyObj.data.name); if (e) e.mesh.visible = false; return; }
    }
    ...
    removeBody(name) {
        ...
        if (!e) return;
        this.ctx.scene.remove(e.mesh); e.material.dispose(); this.overlays.delete(name);
    }
    dispose() { this.overlays.forEach(({mesh, material}) => { this.ctx.scene.remove(mesh); material.dispose(); }); this.overlays.clear(); }
}