...
export class EclipseEngine {
    ...
    static getCandidatePairs(targetData, allBodies) {
        ...
        const pairs = [];
        const addPair = (s, o) => {
            if (s && o && s.name !== o.name) pairs.push({ shadowed: s, occulter: o });
        };
        ...
        if (isMoon) {
            return pairs.filter(
                (p) => p.shadowed.name === targetData.name || p.occulter.name === targetData.name
            );
        }
        return pairs;
    }
    ...
    static findNextEclipse(targetData, allBodies, fromDays, direction = 1) {
        ...
        let prevSignals = pairs.map(
            (p) => this._signalFromSnapshot(p.shadowed, p.occulter, star, prevSnapshot).signal
        );
        ...
        while (Math.abs(t - fromDays) < MAX_HORIZON_DAYS && steps < 100000) {
            ...
            for (let i = 0; i < pairs.length; i++) {
                const { signal, type } = this._signalFromSnapshot(
                    pairs[i].shadowed,
                    pairs[i].occulter,
                    star,
                    snapshot
                ...
                if (prevSignals[i] <= 0 && signal > 0) {
                    ...
                    for (let k = 0; k < 40; k++) {
                        ...
                        const s = this._signalFromSnapshot(
                            pairs[i].shadowed,
                            pairs[i].occulter,
                            star,
                            midSnap
